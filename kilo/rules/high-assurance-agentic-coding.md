...
11. **Always end with a user handoff** — every mode session must produce a user-actionable next-steps handoff using the `user-handoff` skill and the corresponding template in `.kilo/templates/`.
12. **Extension Alloy models must `open` the core model** — do not re-declare shared types (`Player`, `RotationMode`, `Game`, etc.) in extension `.als` files. Use `open <core_model>` to inherit the canonical sig definitions. This prevents silent type divergence and enables cross-model assertion checking.

→ Full handoff detail: `.kilo/rules/user-actionable-output.md`
